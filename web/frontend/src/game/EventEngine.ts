...
export class EventEngine {
  ...
  private applyNextStepPacket(event: RunEvent): void {
    ...
    this.nextStepPacket = {
      ...
      acceptanceCriteria: arrayOfStrings(packet.acceptance_criteria ?? event.acceptance_criteria),
      evidenceRequired: arrayOfStrings(packet.evidence_required ?? event.evidence_required),
      demoRequested: packet.demo_requested === true || event.demo_requested === true,
      demoCompatibility: stringOrNull(packet.demo_compatibility ?? event.demo_compatibility),
  ...
  private applyFunctionalEvent(event: RunEvent, type: string): void {
    ...
    if (type === "mini_sprint_planned") {
      const sprint: FunctionalMiniSprint = {
        ...
        demoAdapter: stringOrNull(event.demo_adapter),
        demoConfidence: stringOrNull(event.demo_confidence),
        demoReason: stringOrNull(event.demo_reason),
  ...
  private applyDemoPlanEvent(event: RunEvent, status: "planned" | "skipped"): void {
    ...
    const next: DemoPlan = {
      ...
      demoCompatibility: stringOrNull(event.demo_compatibility ?? plan.demo_compatibility),
      demoRequested: event.demo_requested === true || plan.demo_requested === true,
  ...
  private applyDemoCaptureEvent(
    ...
    const next: DemoPlan = {
      ...
      demoCompatibility: stringOrNull(event.demo_compatibility) ?? base.demoCompatibility,
      demoRequested: base.demoRequested || event.demo_requested === true,
...
function cloneNextStepPacket(packet: NextStepPacket): NextStepPacket {
  return {
    ...
    demoRequested: packet.demoRequested,
    demoCompatibility: packet.demoCompatibility,
