...
- [ ] Manual: gauges track real activity
...
1. Dry-run canned responses don't match real output → Phase 7 dry-run is for pipeline validation only, not output quality
2. Metrics sparkline rendering blocks UI thread → Use requestAnimationFrame and offscreen canvas
3. Inspection CLI depends on specific .orchestrator/ layout → Document format contract; add tests for edge cases
