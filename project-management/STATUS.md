...
| **H5 Pool Live Wiring** | In progress | Claude | H5.1 shipped 2026-04-21: `SyncPoolClient` wraps `UpstreamPool` so roles' sync `self.llm.chat(...)` reaches the pool without TypeError. H5.2 shipped 2026-04-21: declarative `endpoints:` + `routing:` YAML block (free-form operator labels, per-role model override, env overrides) consumed by both `cli/run.py` and `web/backend/runner.py`; `LLMClient` parameterised on `model`/`api_key`/`extra_body`/`default_headers`; multi-provider (llama.cpp / OpenRouter / OpenAI / Ollama) is now a config choice. H5.3 (end-to-end two-endpoint wire-up test against real live endpoints) still pending |
