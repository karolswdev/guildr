...
| **H6 Opencode Agent Runtime** | In progress | Claude | H6.0 spike shipped 2026-04-21 (`docs/research/opencode-runtime.md`): `opencode run --format json` streams NDJSON events on stdout and `opencode export <sessionID>` dumps the canonical session (messages, parts, tokens, cost) — supersedes direct `/v1/chat/completions` path. H6.1 shipped 2026-04-21: new `orchestrator/lib/opencode_config.py` generates a per-project `.orchestrator/opencode/opencode.json` from the same H5.2 `endpoints:` block; every distinct `(endpoint, model)` reachable via routing is registered. H6.2+ (OpencodeSession adapter, role rewrites, audit-trail replacement, gate-between-sessions) pending |
