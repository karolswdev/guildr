...
| **H5 Pool Live Wiring** | In progress | Claude | H5.1 shipped 2026-04-21: `SyncPoolClient` wraps `UpstreamPool` so roles' sync `self.llm.chat(...)` reaches the pool without TypeError; engine builds one per-role via `Orchestrator._llm_for`; `call_id` now threads through `self.llm.chat` into `pool.jsonl`. H5.2 (parse `PRIMARY_URL`/`ALIEN_URL` env + construct pool in `runner.py` / `cli/run.py`) + H5.3 (end-to-end two-endpoint wire-up test) still pending |
