...
- [ ] Snapshot rate card per run start into `.orchestrator/costs/rate-cards/<date>.json`; replay reads from snapshot, never current rates.
  - [x] Local/on-prem cost profiles are snapshotted write-once into `.orchestrator/costs/rate-cards/local-<machine>-<ts>.json` the first time a project observes them.
  - [x] Usage rows that carry `rate_card_version` also carry replay-safe `rate_card_ref`; the PWA Economics sheet folds versions/refs into `CostSnapshot`.
  - [x] Runtime usage emitters validate referenced rate-card snapshots, stamp `rate_card_checked` / `rate_card_missing`, and downgrade missing estimated costs to unknown.
...
- 2026-04-23 M09 slice C0: local rate-card snapshots landed. `load_local_cost_profile()` now writes a first-observation local profile snapshot under `.orchestrator/costs/rate-cards/`, preserves existing snapshots without mutation, and records env/default local cost assumptions through the same `rate_card_version` used in `usage_recorded`. Evidence: `uv run pytest -q tests/test_rate_card_snapshot.py tests/test_opencode_audit.py tests/test_budget.py` -> 18 passed; `uv run python -m py_compile orchestrator/lib/local_cost.py`; `git diff --check` -> clean.
- 2026-04-23 M09 slice C1: rate-card refs now travel with usage and replay. `_usage_payload()` stamps `rate_card_ref` beside `rate_card_version`, `EventEngine` folds rate-card versions/refs/missing markers into `CostSnapshot`, and the PWA Economics sheet renders the replay dependency list. Evidence: `uv run pytest -q tests/test_rate_card_snapshot.py tests/test_opencode_audit.py tests/test_budget.py` -> 18 passed; `uv run pytest -q web/frontend/tests/test_event_engine.py web/frontend/tests/test_game_map.py` -> 27 passed; `uv run pytest -q tests/test_usage_writer.py tests/test_usage_summary.py tests/test_opencode_audit.py tests/test_rate_card_snapshot.py` -> 21 passed; `./web/frontend/build.sh` -> `dist/app.js` 1,361,560 bytes.
- 2026-04-23 M09 slice C2: rate-card snapshot validation landed. Central LLM/advisor usage and opencode audit now annotate referenced rate-card snapshots before event emission and JSONL persistence. Existing refs get `rate_card_checked=true`, missing/unsafe estimated refs get `rate_card_missing=true` and are downgraded to `source=unknown`, `confidence=none`; the PWA folds and renders missing versions. Evidence: `uv run pytest -q tests/test_rate_card_snapshot.py tests/test_opencode_audit.py tests/test_budget.py tests/test_usage_writer.py tests/test_usage_summary.py` -> 29 passed; `uv run pytest -q web/frontend/tests/test_event_engine.py web/frontend/tests/test_game_map.py` -> 27 passed; `./web/frontend/build.sh` -> `dist/app.js` 1,361,560 bytes; `git diff --check` -> clean.
