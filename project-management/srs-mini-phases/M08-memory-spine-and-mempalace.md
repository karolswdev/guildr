...
- [x] Optional after provenance is green: generate OpenCode `mcp.mempalace` config using `python -m mempalace.mcp_server`, disabled globally and enabled only for selected tool-using agents. Landed 2026-04-22: `memory_mcp.enabled` plus `roles` emits local MCP config for coder/tester/reviewer/narrator only; architect/judge are rejected.
...
- 2026-04-22 M08 slice D2: opt-in MemPalace MCP config landed. `orchestrator.lib.endpoints.MemoryMcpConfig` parses `memory_mcp.enabled`, selected roles, command, timeout, and environment from YAML/env. `orchestrator.lib.opencode_config` emits local `mcp.mempalace` config only when enabled, disables `mempalace_*` globally, and re-enables it per selected role. Allowed roles are coder/tester/reviewer/narrator; architect/judge are rejected so zero-tool roles remain deterministic. Evidence: `uv run pytest -q tests/test_endpoints.py tests/test_opencode_config.py tests/test_cli_run.py tests/test_integration_h6_opencode_pipeline.py` -> 30 passed.
